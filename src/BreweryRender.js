...
class BreweryRender extends React.Component {
  ...
  showBreweries = () => {
    const allFilteredBrews = this.props.firstFilter.filter(brew =>
      this.props.secondFilter.includes(brew)
    );
    const currentBrews = allFilteredBrews.slice(
      this.state.index,
      this.state.index + 40
    );
    return currentBrews.map(brewery => (
      <li className="brewLink" key={brewery.id}>
        {" "}
